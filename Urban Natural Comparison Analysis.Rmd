...
Note- Observations are the average of 1-8 scans conducted at 10 minute intervals. 
There are 8 observations from natural wetlands and 9 from urban parks. 
...
print(aggregated) #This shows flock scans per observation (site/date combo); 17 rows total
...
What time of day were scans taken during?

```{r}
#this is a super clunky way to doing things 
fs_t<-fs

fs_t$time2 <-as.POSIXlt(fs_t$time, format = "%H:%M")

fs_t$hour<-hour(fs_t$time2)
fs_t$min<-minute(fs_t$time2)

fs_t<-fs_t[
  with(fs_t, order(hour, min)),
]
...
###Create theme for multipanel figure
```{r}
...
theme_fig4 <- function(){ 
    ...
    theme(
     ...
     # plot.subtitle = element_text(          #subtitle
      ...
      axis.title.x = element_blank(),              #font size
...
geom_pointrange(position = position_jitterdodge(seed=40,dodge.width = 0.0, jitter.width = 0.7),aes(color=site_type,fill=site_type))+ geom_boxplot(outlier.shape=8,outlier.color="black",alpha = 0.4,aes(fill=site_type))+theme_fig4()+
...
geom_pointrange(position = position_jitterdodge(seed=40,dodge.width = 0.9, jitter.width = 1),aes(color=site_type,fill=site_type))+  geom_boxplot(outlier.shape=8,outlier.color="black",alpha = 0.4,aes(fill=site_type))+theme_fig4()+
...
geom_pointrange(position = position_jitterdodge(seed=40,dodge.width = 0.9, jitter.width = 1),color="black",shape=21,aes(fill=site_type)) + geom_boxplot(outlier.shape=8,outlier.color="black",alpha = 0.4,aes(fill=site_type))+theme_fig4()+
...
bar2<- ggplot(data=fs_bar, aes(x=site_type, y=mean, fill=Activity2)) +
  geom_bar(stat="identity") + scale_fill_viridis(discrete=TRUE)+theme_fig4()+ ggtitle("Activity Budgets") #most common on top

bar2_leg<- ggplot(data=fs_bar, aes(x=site_type, y=mean, fill=Activity2)) +
  geom_bar(stat="identity") + scale_fill_viridis(discrete=TRUE, name="Behavior")+ ggtitle("Activity Budgets")#same plot but with legend so I can later extract it

```

### Figure 4 for manuscript
```{r}
leg <- get_legend(bar2_leg) #extract barplot legend 

#use cowplot to create multipanel figure
fig4<-plot_grid(NULL,f2plot,g2plot,v2plot,NULL,NULL,r2plot,w2plot,bar2,leg,ncol=5,rel_widths = c(0.1,1,1,1,0.25,0.1,1,1,1,0.25), labels = c('','A', 'B','C','','','D','E','F',''),label_size = 14)

#Cowplot doesn't make it easy to make shared legends to switching back to gridExtra for that
#left<-textGrob("Proportion of flock doing each behavior", rot = 90,gp=gpar(fontsize=12,fontfamily='Arial',fontface="bold"))
left<-textGrob("Proportion of flock doing each behavior", rot = 90,gp=gpar(fontsize=16,fontfamily='Arial'))

bottom<-textGrob("Site type",gp=gpar(fontsize=16,fontfamily='Arial'))

fig4_2<- grid.arrange(fig4,left=left,bottom=bottom) #use gridExtra to make shared axis labels
fig4_2

ggsave2(file="outputs/fig4.png",fig4_2,dpi=300,limitsize = TRUE) #Save figure
...
```{r adjust p vals, echo=TRUE, message=FALSE}
f<-t.test(fs_obs$prop_forage~fs_obs$site_type)$p.value
g<-t.test(fs_obs$prop_groom~fs_obs$site_type)$p.value
...
pvalsDF <- data.frame (first_column  = c("foraging", "grooming", "vigilant","resting","walking"),
                     p.orig=pvals,
                  p.adj= padj)
