...
vid <- vid0
vid<-dplyr::select(vid,vid_ID,site,date,vid_length,focalID,count_2m=`2m_density`,flock_size)
vid$count_2m<-as.numeric(vid$count_2m)
vid$uniqueID <- paste(vid$vid_ID,vid$vid_length,vid$focalID,sep='')
vid$vid_ID<-NULL #remove old ID

vid_avg<-vid %>%drop_na() %>%  #drop rows with NA
  group_by(uniqueID) %>% 
  dplyr::summarise(avg_count=mean(count_2m)) %>% #average by uniqueID 
  dplyr::rename(focalID=uniqueID)
...
FF_avg<-FFprox_1m %>%drop_na() %>%  #drop rows with NA
   group_by(focalID) %>% 
   dplyr::summarise(avg_count=mean(count_2m))  
...
ggplot(count_1m, aes(x=site, y=avg_count, fill=foraging_type)) +
  ...
  xlab("Site") + ylab("Number of conspecifics within 1m of focal ibis") #by site

ggplot(count_1m, aes(x=foraging_type, y=avg_count)) +
  geom_boxplot(position=position_dodge(1)) + theme_classic() +
  ylab("Number of conspecifics within 1m of focal ibis") + theme(axis.title.x=element_blank()) #all
...
#plot for Figure 2:
fig2<-ggplot(count_1m, aes(x=foraging_type, y=avg_count)) +
  geom_boxplot(outlier.shape=8,notch=FALSE) + theme_classic() +
  ylab("Number of conspecifics within 1m of focal ibis") + theme(axis.title.x=element_blank(),legend.position="none")+geom_jitter(alpha = 0.8,shape=16, position=position_jitter(0.3,seed=40),aes(color=foraging_type,fill=foraging_type))+ 
  scale_color_manual(values=c("#21918c", "#440154"))+
  scale_fill_manual(values=c("#21918c", "#440154"))

ggsave(file="outputs/fig2.png",fig2,height=4,width=6,units='in',limitsize = TRUE) #Save figure
