...
zinbm1.int = glmmTMB(Foraging~flocksize_nearest_s*feedingperhour_s +offset(log(duration)), zi=~1, ForagePreen_wide, family=nbinom2) #intercept only zero inflated term


AICtab(pm1,nbm1,zipm1,zipm1.int,zinbm1,zinbm1.int) #zinbm1.int is best fit but does this make sense? can you have an intercept only term in the zi part of the model? 
...
#Now let's look at fit of neg binom model to handle zero inflation:
...
#how does a neg binom zero inflated model look? compare zinbm1 and zinbm1.int
...
Now build the same models but include site as a random effect and see if same models are best fitting. 
...
```{r}
predict(fmod13,ForagePreen_wide,se.fit=TRUE)
...
```{r}
predict(fmod17,ForagePreen_wide,se.fit=TRUE)
...
ggsave("foragingmodeloutput.png")
...
### Grooming models: 
 ...
 Preening includes preening or grooming (bathing). 
