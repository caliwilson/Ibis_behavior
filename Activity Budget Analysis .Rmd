...
grouped_summary<-ab_summary %>% 
 ...
 dplyr::summarise(prop.new = sum(prop, na.rm = TRUE))

checking<-grouped_summary %>% #check to make sure all props sum to 1
  group_by(focalID) %>% 
 dplyr::summarise(prop.new2 = sum(prop.new, na.rm = TRUE))
...
all_urban<-grouped_summary %>% 
  group_by(act_type) %>% 
 dplyr::summarise(prop.new = mean(prop.new, na.rm = TRUE))
...
flocksize.bydate<- flocksize %>% 
  dplyr::group_by(site,date) %>%dplyr::summarise(size=mean(WHIB_total))
...
WaterExposure <-WaterExposure  %>% 
 ...
 dplyr::summarise(activity_count = sum(activity_count, na.rm = TRUE))
