...
p1<-ggplot(fmod17_predict, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = .1)+xlab("Provisioning Frequency (standardized)") + ylab("Predicted Foraging Time")+ggtitle("flock_size + provisioning + (1 | site)")+theme_classic()

p2<-ggplot(fmod17_predict3, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = .1)+xlab("Flock Size (standardized)") + ylab(NULL)+theme_classic()
...
ggplot(ForagePreen_wide, aes(x=feedingperhour_s, y=Foraging)) + geom_point(color='dimgray') + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  theme(plot.title = element_text(face = "bold")) + 
  labs(title="flock_size + provisioning + (1 | site)",
      y="Foraging time (seconds)",
      x="Provisioning Frequency (standardized") +
  geom_line(data=fmod17_predict, aes(x = x, y = predicted)) +
  geom_ribbon(data=fmod17_predict,aes(ymin = conf.low, ymax = conf.high), alpha = .1)+ theme_classic()
...
p1_points_unscaled<-ggplot(fmod17_predict_unscaled, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = .2,fill='#440154')+xlab("Provisioning Frequency") + ylab("Foraging Time (seconds)")+ggtitle("flock_size + provisioning + (1 | site)")+theme_classic()+geom_point(data=ForagePreen_wide, aes(x=feedingperhour, y=Foraging),color='#ABABAB')

p2_points_unscaled<-ggplot(fmod17_predict2_unscaled, aes(x, predicted)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = .2,fill='#440154')+xlab("Flock Size") + ylab("Foraging Time (seconds)")+theme_classic()+geom_point(data=ForagePreen_wide, aes(x=flocksize_nearest, y=Foraging),color='#ABABAB')
...
```{r}
Cand.models <- list("null" =fmod0.2, 
                    "provisioning" = fmod15,
                    "flock size" = fmod16,
                    "provisioning + flock size" = fmod17,
                    "provisioning * flock size" = fmod18)
selectionTable <- aictab(cand.set = Cand.models, second.ord = FALSE) #uses AIC 
selectionTable

library(xtable)
ftable<-print(xtable(selectionTable, caption = "Model selection table on ibis foraging time.",
label = "tab:selection"),
...
```{r}
Cand.models_groom <- list("null" =pmod0.2, 
                    "provisioning" = pmod15,
                    "flock size" = pmod16,
                    "provisioning + flock size" = pmod17,
                    "provisioning * flock size" = pmod18)
selectionTable_groom <- aictab(cand.set = Cand.models_groom, second.ord = FALSE) #uses AIC 
selectionTable_groom

library(xtable)
gtable<-print(xtable(selectionTable_groom, caption = "Model selection table on ibis grooming time.",
label = "tab:selection"),
include.rownames = FALSE, caption.placement = "top")
library(texPreview)
tex_preview(gtable)
...
```{r}
Cand.models_vig <- list("null" =vmod0.2, 
                    "provisioning" = vmod15,
                    "flock size" = vmod16,
                    "provisioning + flock size" = vmod17,
                    "provisioning * flock size" = vmod18)
selectionTable_vig <- aictab(cand.set = Cand.models_vig, second.ord = FALSE) #uses AIC 
selectionTable_vig

library(xtable)
vtable<-print(xtable(selectionTable_vig, caption = "Model selection table on ibis vigilance time.",
label = "tab:selection"),
include.rownames = FALSE, caption.placement = "top")
library(texPreview)
tex_preview(vtable)
